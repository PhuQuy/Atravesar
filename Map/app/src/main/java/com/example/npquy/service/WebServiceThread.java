	...
	private HttpResponse doResponse(String url) {
		...
		try {
			...
			case POST:
				...
				// Log.i(TAG,"HTTP Entiry : " +
				// convertStreamToString(httpPost.getEntity().getContent()));
/*				StopHandler stopHandler = StopHandler.createInstance();
				int run = 0;
				for (int i = 0; i < 5; i++) {
					if (stopHandler.isRunning() == 0) {
						run++;
						Thread.sleep(1000);
					}
				}
			...
			case GET:
				...
				int num = StopHandler.createInstance().isRunning();
				System.out.println(Thread.currentThread().getName() + " " +num);
				Thread.currentThread().sleep(2000);
				System.out.println(Thread.currentThread().getName() + " " +StopHandler.createInstance().isRunning());
				if (num == StopHandler.createInstance().isRunning()) {
